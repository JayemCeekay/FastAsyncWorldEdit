...
//FAWE end
    ...
    */
    ...
    default IChunkSet processSet(IChunk chunk, IChunkGet get, IChunkSet set, boolean asBlacklist) {
        ...
        if (tx >= min.x() && bx <= max.x() && tz >= min.z() && bz <= max.z()) {
            ...
            for (int layer = minLayer; layer <= maxLayer; layer++) {
                ...
                processExtra = shouldProcessExtra(set, processExtra, layer, set.load(layer));
            }
            if (processExtra) {
                BlockVector3 chunkPos = chunk.getChunkBlockCoord().withY(0);
    ...
    private boolean shouldProcessExtra(IChunkSet set, boolean processExtra, int layer, DataArray arr) {
        for (int y = 0, index = 0; y < 16; y++) {
            for (int z = 0; z < 16; z++) {
                for (int x = 0; x < 16; x++, index++) {
