    ...
    public T create(ChunkPacket packet) {
        try {
            ...
            if (fieldHeightMap != null) {
                Object heightMap = FabricWorldEdit.inst.getFaweAdapter().fromNativeLin(packet.getHeightMap().toLinTag());
            ...
            for (Map.Entry<BlockVector3, CompoundTag> entry : tiles.entrySet()) {
                Object nmsTag = FabricWorldEdit.inst.getFaweAdapter().fromNativeLin(entry.getValue().toLinTag());
                nmsTiles.add(nmsTag);
            }
