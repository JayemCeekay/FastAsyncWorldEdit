...
public class PlotSquaredFeature extends FaweMaskManager {
    ...
    private boolean playerOnline(UUID uuid) {
        ...
        ServerPlayer player = FabricPlatform.SERVER.getPlayerList().getPlayer(uuid);
        return player != null;
    }

    @Override
    public FaweMask getMask(Player player, MaskType type, boolean isWhitelist) {
        final PlotPlayer<ServerPlayer> pp = PlotPlayer.from(FabricPlatform.SERVER.getPlayerList().getPlayer(player.getUniqueId()));
        if (pp == null) {
            return null;
        }
