...
public class FabricNMSRelighter implements Relighter {
    ...
    public FabricNMSRelighter(IQueueExtent<IQueueChunk> queue) {
        this(queue, null);
    }

    public FabricNMSRelighter(IQueueExtent<IQueueChunk> queue, RelightMode relightMode) {
        this.queue = queue;
    ...
    public void updateBlockLight(Map<Long, long[][][]> map) {
        ...
        while (!lightPropagationQueue.isEmpty()) {
            ...
            if (BuiltInRegistries.BLOCK.get(ResourceLocation.parse(id)) instanceof SlabBlock){
            ...
            } else if (BuiltInRegistries.BLOCK.get(ResourceLocation.parse(id)) instanceof StairBlock) {
    ...
    private void computeStair(
        ...
        east:
        {
            ...
            if (!(FabricAdapter.adapt(state.getBlockType()) instanceof StairBlock)) {
    ...
    private boolean isStairOrTrueTop(BlockState state, boolean top) {
        return !(FabricAdapter.adapt(state.getBlockType()) instanceof StairBlock) || state.getState(stairHalf).equals("top") == top;
    }

    private boolean isSlabOrTrueValue(BlockState state, String value) {
        return !(FabricAdapter.adapt(state.getBlockType()) instanceof SlabBlock) || state.getState(slabHalf).equals(value);
    }
